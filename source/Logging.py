...
process_logger = lambda process_name, ratio: print(colored(time_logging(datetime.now()) +
                                                           '\t{:.2f}% of {} is completed.'.format(ratio*100, process_name),
                                                           'cyan')) if True in [round(ratio*100, 1) % number == 0
                                                                                for number in range(9, 12)] else ''
